...
static GOptionEntry entries[] = {
    ...
    {"maxrows", 'r', 0, G_OPTION_ARG_INT, &opt_maxrows,
     "Max number of rows to draw", "N"},
    {"maxtextlen", 'l', 0, G_OPTION_ARG_INT, &opt_maxtextlen,
     "Max length of a text command", "N"},
    {"file", 'f', 0, G_OPTION_ARG_FILENAME, &opt_filename,
     "file to load instead of using i3-mesg", "M"},
    {G_OPTION_REMAINING, ' ', 0, G_OPTION_ARG_FILENAME_ARRAY, &opt_remaining,
     NULL, NULL},
    {NULL}};
...
void print2(gchar *b) { g_print(b); }

void print_binding(struct binding *b) { g_print("%s - %s\n", b->keys, b->cmd); }

GSList *parse(gchar *all) {
  gchar **line = NULL, **lines = NULL, **args = NULL;
  GSList *results = NULL;
  struct binding *t;
  gboolean inMode = FALSE, isModeTitle = FALSE;
...
void addPageLabel(GtkWidget *grid, int cols) {
  ...
  gtk_label_set_markup(
      GTK_LABEL(label),
      "<span foreground='#808080'>&lt;space&gt; : next page</span>");
...
static void activate(GtkApplication *app, gpointer user_data) {
  ...
  g_signal_connect(G_OBJECT(window), "key_press_event",
                   G_CALLBACK(keypress_function), app);
  ...
  printNextPage();
  GdkColor color;
  if (gdk_color_parse("#141C21", &color)) {
...
char *replaceDelimiters(const gchar *text, const gchar *delims,
...
gint ceil_div(gint x, gint y) { return x / y + (x % y != 0); }

GSList *join(GSList *lines) {
  GSList *results = NULL;
  gchar *result = "";
  gint total = g_slist_length(lines), colSize = ceil_div(total, opt_columns),
       cols = opt_columns, colMod = colSize != opt_maxrows ? total % cols : 0,
       row = 0, col = 0;
  ...
  while (lines != NULL) {
    ...
    row++;
    if (lines == NULL ||
        row == (colSize - ((colMod > 0 && col >= colMod) ? 1 : 0))) {
